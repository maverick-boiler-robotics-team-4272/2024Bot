...
public class TestAutoCommand extends SequentialCommandGroup {
    public TestAutoCommand(Drivetrain drivetrain) {
        super(
            new PathFollowState(drivetrain, getGlobalTrajectories().TEST_PATH.trajectory, false, true, DEFAULT_POSE_DELTA, true, false),
            new PathFollowState(drivetrain, getGlobalTrajectories().TEST_PATH.trajectory, false, true, DEFAULT_POSE_DELTA, false, false).repeatedly()
        );
    }
}
